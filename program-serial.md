...
- **`bash run.sh` = one screening trial; `bash run.sh 8` = the fixed 8-trial
  ...
  use which.
- **Wait for each run to finish, then act on its result.** Edit the code
  yourself, launch the run, and wait until it completes or fails; then check it
  with `python verify.py logs/<uuid>.txt`. There is nothing useful to do while
  it runs — don't tail logs or poll `nvidia-smi`.
